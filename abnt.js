...
async function abnt() {
  ...
  async function getList(page) {
    try {
        ...
        if (stop === false) {
          for (let j = 0; j <= getMacrossetor.length - 1; ) {
            ...
            if (
              ...
              if (
                getProjectsFunc[i - 1].macrossetor ===
                getMacrossetor[j].macrossetor
              ) {
                getScript.push({
                  macrossetor: getProjectsFunc[i - 1].macrossetor,
                  projeto: gettingProject,
                });
                gettingProject = [];
              }
      ...
      getScript.forEach((data) => {
        data.projeto.forEach((project) => {
          delete project.macrossetor;
        });
      });
      //Add total projects of abnt
      getScript.push({ projetosAtivos: getProjectsFunc.length });
      //Write a JSON file
      fs.writeFile(
        path.resolve(__dirname, "abtn.json"),
        JSON.stringify(getScript, null, 2),
        (err) => {
          if (err) {
            console.log(err);
          }
